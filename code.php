...
if (isset($_POST['export_excel_btn'])) {
    ...
    if ($T == '1') {
        $sql2 = "SELECT SUM('MONTT') as 'D' FROM `EXCEL` WHERE `DC`='D';";
        $query_run3 = mysqli_query($con, $sql2) or die("bad query");
        while ($sumC = $query_run3->fetch_object()) {
            $D = $sumD->D;
        }
        $sql3 = "SELECT SUM(`MONTT`) as 'C' FROM `EXCEL` WHERE `DC`='C';";
        $query_run4 = mysqli_query($con, $sql3) or die("bad query");
        while ($sumC = $query_run4->fetch_object()) {
            $C = $summC->C;
        }
        $sql4 = "SELECT `MONTT` FROM `EXCEL` WHERE `Code_enregistrement` LIKE '%7%';";
        ...
        if ($MTT == $V) {
            ...
            $_SESSION['message'] = " Montant Invalid $V ";
