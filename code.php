        ...
        if (abs($MT) == abs($q)) {
            ...
            if ($query_run) {
                ...
                while ($row1 = $query_run->fetch_assoc()) {
                    ...
                    $ff = str_pad($row1['substring(Code_enregistrement,2,2)'], 2, "0", STR_PAD_LEFT) . "" . str_pad($row1['substring(Code_banque,1,5)'], 5, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(Code_interne,1,4)'], 4, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(Code_guichet,1,5)'], 5, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(Devise,1,3)'], 3, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(Indice_d_exaunération,1,1)'], 1, " ", STR_PAD_RIGHT) . " " . str_pad($row1['substring(RIB,1,11)'], 11, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(CIB,1,2)'], 2, " ", STR_PAD_RIGHT) . "" . str_pad($row1['Date_operation'], 6, " ", STR_PAD_RIGHT) . "  " . str_replace('000000', '      ', str_pad($row1['Date_devaleur'], 6, " ", STR_PAD_RIGHT)) . "" . str_pad($row1['substring(Libellé,1,31)'], 31, " ", STR_PAD_RIGHT) . "" . str_pad($row1['substring(Référence,1,11)'], 11, " ", STR_PAD_RIGHT) . "" . $row1['substring(Montant,1,30)'] . "000402016" . str_pad($row1['SENS'], 7, " ", STR_PAD_RIGHT) . "\n";
